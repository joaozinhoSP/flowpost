-- Migration: FlowPost - Schema completo
...
-- 6. Função auxiliar para atualizar updated_at
...
-- 7. Trigger para updated_at em social_accounts
CREATE TRIGGER set_social_accounts_updated_at
  BEFORE UPDATE ON social_accounts
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();

-- 8. Função: buscar posts para publicar (entre agora e +30min)
...
-- 9. Função: atualizar status do post após publicação
...
-- 10. Função: obter token de acesso de plataforma
